...
function save_history(day, profile) {
  if (day == undefined)
    return;
...
export function backup_today(today, profile) {
  ...
  if (Date.now() - today.server_checked < check_backup_interval)
    return;
...
export function backup_favorites(favorites, profile) {
  ...
  if (Date.now() - favorites.server_checked < check_backup_interval)
    return;
